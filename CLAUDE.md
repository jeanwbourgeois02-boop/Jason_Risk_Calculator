...
2. **Missing stays missing, and a missing price is filled from the trade's own last close.** No fallback mark source for P&L or delta, ever, and nothing is written to `marks` that Bloomberg or the app's own pricers did not produce. On every screen a trade with no price on a date takes its own valuation from the last earlier business day that has one, at most 5 back, and its row says so (user decision 2026-09-21: "there should be a fill when bloomberg doesnt have the data"; see "The fill" under "Header"). With no earlier price in reach it has blank P&L and a plain-language reason shown where the number would be: never zero, never a silent drop. A stored value that is not a number is a data error, never filled. One other exception, by user decision: a settled NDF with no past fix on file (see "Settled trades are frozen").
...
- **An NDF that has fixed is handled like settled cash** (user decision 2026-09-21: "the NDF ticket that fixes out should be handled like settled cash, where the Local amnt stays in 'settled' and be priced using spot rate"). From its fixing date until its value date, both legs sit in the Settled cash row at face value in their own currency (`exposure_adapter.ndf_fixed_records`), the local amount valued at the currency's official spot, never the 1M NDF price, and it carries that currency's delta like deliverable settled cash (in the delta columns at the currency's one rate, the 1M price). Boundary as for deliverable cash, read on the fixing date: the grid keeps a leg fixing on `as_of` on its own date, the delta counts it as settled by close. After the value date the ledger realises it and the row shows its USD settlement from `realised_pnl`, as before. P&L follows the same days (see "Mark date").
...
- **Settled cash row**, on top (user decision 2026-09-18: "expired tickets must settle not disappear"; `engine/ladder/exposure_adapter.py::settled_records_from_db` is its only source). Deliverable legs past their value date sit there at face value in their own currency and still carry that currency's delta (NOK received on a settled forward is NOK exposure until sold). Non-deliverable tickets (NDF pairs past their value date, futures, FX options) contribute their USD settlement read from `realised_pnl`, never recomputed; one the ledger has not realised yet is named under the grid, not valued. This is settled cash from the tickets on file, not a bank balance.
- The tab's per-currency delta therefore includes settled deliverable cash, and deliverable legs settling on `as_of` count as cash by close; a fixed NDF's legs count from the fixing date through the value date (above) and carry none once that has passed. The SQL delta query below and the per-pair Position table stay open-forward-only. Delta rows are at spot (an NDF currency at its 1M NDF price). Gold keeps its own sign in the dollar-convention column (a metal is not a dollar position).
...
- **Mark date**: each FX leg is marked with the `FWD_OUTRIGHT` for its own `settle_date` (not a single T+5 date). One exception, NDF tickets only (user decision 2026-09-21, `engine/pnl/valuation.py::_ndf_at_spot`): from its fixing date (value date less 2 business days, the Ladder's rule) until its value date an NDF is marked at the pair's official SPOT of the date valued, `Q × (S_pair − f)` converted at spot as always, and its note says so; an NDF that has not fixed but has no official forward for its value date on that date is marked at that day's spot the same way. After the value date the ledger freezes it as it always has. A deliverable ticket with no forward stays blank.
