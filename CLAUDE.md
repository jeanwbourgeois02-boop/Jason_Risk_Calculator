...
- Layout (user decision 2026-09-21): the per-currency summary (FX rate as quoted, Local delta, USD delta, Rate source; currencies across) is its own table on top; the grid beneath it has one row per currency and the dates across (Settled cash first, then the dates, then the total of the columns shown), with the USD equivalent as its bottom row.
...
- **Mark time**: official close is 15:00 `America/New_York` (user decision 2026-09-21: "the EOD is 3pm New York time", for Daily, 5d, MTD and every past FX close; it replaced the 17:00 close of 2026-09-15). Bloomberg's daily `PX_LAST` is its 17:00 close, so a past FX close is read from Bloomberg's intraday bars at 15:00 New York instead and stamped 15:00; a past-day FX row stamped at any other time (an earlier live press, an old 17:00 row) is not a close and the backfill replaces it. Futures keep `PX_SETTLE`. The app resolves the offset from the zone per date; intraday = live. Every mark row carries `snapped_at` with the resolved offset for that row.
