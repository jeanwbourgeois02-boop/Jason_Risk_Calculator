...
- **Settled trades are frozen.** A trade whose last leg has settled takes its row from `realised_pnl` and is never marked again; settlement does not move LTD. The freeze is at the pair's last official SPOT on or before the settlement date. The one exception to hard rule 2 (user decision 2026-09-21: "we can use a present spot for the past fixes"): an NDF ticket (`engine/ladder/ndf.py::is_ndf_pair`) whose pair has no official SPOT on or before its settlement is valued at the latest official SPOT on file, and its note names that date. It is frozen there once, by the backfill's closing step only (`realise_settled(ndf_present_spot=True)`), after the backfill has tried for the settlement date's own close; the live pull's own freeze stays strict. A deliverable trade in the same position stays blank.
