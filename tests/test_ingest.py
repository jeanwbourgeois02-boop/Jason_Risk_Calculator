...
2026-09-24 (commodity conversion Phase 2): the FX-swap package rule (data/ingest/swaps.py
and tests/test_swaps.py), the swap direction overrides (data/ingest/irs_direction.py), the
swap pricer's official marks and the NDF marks left the app, and their tests here with them.
...
def test_official_mark_source_is_exactly_the_contract_table():
    """CLAUDE.md "Official marks", after the 2026-09-24 removal of rates and NDFs: the swap
    pricer's PAR_RATE / PV_USD / DV01_USD / CASHFLOW_USD and the NDF_1M / NDF_FIX marks are
    no longer mark types, so none of them has an official source."""
    assert schema.OFFICIAL_MARK_SOURCE == {
        "SPOT": "BBG_BFXFORWARD",
...
def test_marks_official_shows_no_row_of_a_retired_mark_type_whatever_its_source():
    """An old database keeps its swap and NDF marks; they simply stop being official."""
    conn = schema.connect()
    conn.execute("INSERT INTO instruments VALUES ('USDKRW','FX','USD','KRW',1,0,'USDKRW Curncy','9999-12-31')")
    retired = [("PV_USD", "QL_PRICER"), ("PV_USD", "BBG_BDH"), ("DV01_USD", "QL_PRICER"),
               ("PAR_RATE", "QL_PRICER"), ("CASHFLOW_USD", "QL_PRICER"),
               ("NDF_1M", "BBG_BFXFORWARD"), ("NDF_FIX", "BBG_BDH")]
    conn.executemany("INSERT INTO marks VALUES (?,?,?,?,?,?,?)",
                     [("2026-09-17", "USDKRW", "2026-09-17", mt, 1.0, src, "t") for mt, src in retired]
                     + [("2026-09-17", "USDKRW", "2026-09-17", "SPOT", 1390.0, "BBG_BFXFORWARD", "t")])
    got = conn.execute("SELECT mark_type, source FROM marks_official").fetchall()
    assert got == [("SPOT", "BBG_BFXFORWARD")]


def test_new_database_has_no_retired_tables():
    """2026-09-24: swap_review (FX-swap package rule), irs_direction_overrides and
    index_fixings (swaps) are not created on a new database, nor exported as schema names."""
    conn = schema.connect()
    tables = {r[0] for r in conn.execute("SELECT name FROM sqlite_master WHERE type='table'")}
    assert not tables & {"swap_review", "irs_direction_overrides", "index_fixings"}
    assert "index_fixings" not in schema.TABLES
    for gone in ("IRS_DIRECTION_DDL", "IRS_DIRECTION_TABLES", "_SWAP_REVIEW_DDL"):
        assert not hasattr(schema, gone)
...
def test_create_schema_resyncs_a_stale_marks_official_view(tmp_path):
    ...
    try:
        opt = "USDJPY111926P-1"
        conn.execute(f"INSERT INTO instruments VALUES ('{opt}','FX_OPTION','USD','JPY',1,0,'USDJPY Curncy','2026-11-19')")
        conn.executemany("INSERT INTO marks VALUES (?,?,?,?,?,?,?)", [
            ("2026-08-17", opt, "2026-11-19", "DELTA", 0.40, "MANUAL", "t"),
            ("2026-08-17", opt, "2026-11-19", "DELTA", 0.45, "QL_OPTIONS_PRICER", "t"),
            ("2026-08-17", opt, "2026-11-19", "PV_USD", 100.0, "BBG_BDH", "t"),   # retired type
        ])
        got = conn.execute("SELECT mark_type, value, source FROM marks_official").fetchall()
        # re-synced: DELTA to the options pricer, not MANUAL; PV_USD (the old view's
        # BBG_BDH branch) no longer official at all
        assert got == [("DELTA", 0.45, "QL_OPTIONS_PRICER")]
    finally:
        conn.close()
...
def test_purge_drops_a_leftover_swap_review_table_and_leaves_the_other_retired_tables(tmp_path):
    """2026-09-24: an old database's swap_review (FK to trades) is dropped, with its rows;
    index_fixings and irs_direction_overrides stay as they are (never dropped). Idempotent."""
    ...
    conn.executescript("""
        ...
        INSERT INTO swap_review VALUES ('g', 'x-1', 'old');
        CREATE TABLE index_fixings ("index" TEXT NOT NULL, fixing_date TEXT NOT NULL, value REAL NOT NULL,
          source TEXT NOT NULL, PRIMARY KEY ("index", fixing_date, source));
        INSERT INTO index_fixings VALUES ('SOFR', '2026-09-11', 0.0405, 'BBG_BDH');
        CREATE TABLE irs_direction_overrides (trade_id TEXT PRIMARY KEY, direction TEXT NOT NULL, set_at TEXT NOT NULL);
        INSERT INTO irs_direction_overrides VALUES ('s-1', 'RECEIVE', '2026-09-18');
    """)
    schema.create_schema(conn)            # startup on the old database: nothing dropped by the schema itself
    assert conn.execute("SELECT COUNT(*) FROM swap_review").fetchone()[0] == 1

    counts = schema.purge_retired_sources(conn)
    assert counts["swap_review_table_dropped"] == 1
    tables = {r[0] for r in conn.execute("SELECT name FROM sqlite_master WHERE type='table'")}
    assert "swap_review" not in tables
    assert conn.execute("SELECT COUNT(*) FROM index_fixings").fetchone()[0] == 1
    assert conn.execute("SELECT COUNT(*) FROM irs_direction_overrides").fetchone()[0] == 1
    conn.execute("DELETE FROM trades WHERE trade_id = 'x-1'")   # no FK left to block it
    assert schema.purge_retired_sources(conn)["swap_review_table_dropped"] == 0
