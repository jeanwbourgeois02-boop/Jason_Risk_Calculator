...
def test_skipped_no_closes_and_error_days_carry_the_option_keys(tmp_path, monkeypatch):
    ...
    events = []
    _fakes(monkeypatch, events)
    keys = {"options_priced": None, "options_skipped": [], "options_note": ""}
...
def test_status_file_carries_an_options_block_per_worked_day(tmp_path, monkeypatch):
    ...
    _fakes(monkeypatch, events, price_result={"priced": 0, "skipped": skipped})
    ...
    assert block["options"]["2026-09-18"] == {"priced": 0, "skipped": skipped, "note": ""}
    assert block["options"]["2026-09-14"] == {"priced": None, "skipped": [], "note": ""}
