...
There are exactly two entry points: double-click `SETUP.cmd` once; then type `pnl`.

---

## 1. Set up

Double-click `SETUP.cmd` in the repository folder (or wherever you saved it, even before cloning). It takes a
...
- `py -3 risk.py start --force-new` (inside the project folder) always starts a fresh instance.
...
today, read the ladder. Details in [docs/HOW_IT_WORKS.md](docs/HOW_IT_WORKS.md) section 8.
...
|---|---|
| `SETUP.cmd` says winget is unavailable | Update Windows, or install Python 3.12+ / Git manually from python.org and git-scm.com, then re-run `SETUP.cmd`. |
| `'pnl' is not recognized` | Open a **new** PowerShell window (profiles load at startup), or re-run `SETUP.cmd`. |
| `No .venv yet` | Double-click `SETUP.cmd`. |
| `Missing dependency` on start | Double-click `SETUP.cmd`. |
| App opens on 8051, 8052... | An older copy is still running. `doctor` names the port. Close that terminal window. |
| Every USD figure is blank / "Unavailable" | No official Bloomberg marks. This is by design on a PC without a Terminal. On the Bloomberg PC run `py -3 risk.py doctor --bloomberg`. |
...
| Tests fail after `git pull` | Double-click `SETUP.cmd` (new packages may be needed). If they still fail, the commit is broken: report the failing test. |
