# tools/setup.ps1 -- the part of 1_setup.cmd's logic that only makes sense once you already
...
#
# 1_setup.cmd is the one entry point for a brand-new PC (it can also install Python and Git,
...
try {
    ...
    if ($LASTEXITCODE -ne 0) { throw "2_launcher.py setup failed with exit code $LASTEXITCODE" }
