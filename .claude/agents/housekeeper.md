...
5. **Clean up.** After a feature lands, or when the user asks, spawn `infra` (`.claude/agents/infra.md`, the clean-up / infra agent) for the housekeeping no feature owner does: dead code and stale references the change left behind, the launcher / setup / diagnostic scripts at the root, `config/`, `tools/`, the shared test fixtures (`tests/conftest.py`), `.gitignore`. Brief it with the file list of what just changed. It runs after the feature agents, never alongside one in the same files; a deletion inside another agent's files needs that owner's yes, relayed through you, before it is made; its changes go through the same single full-suite run in the next step.
