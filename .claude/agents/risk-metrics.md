...
**Reads** (the lanes whose output you use): book-positions, pnl-series.
...
- The return history is the nm-dashboard market history (`engine/risk/history.py`), read for risk metrics only. Nothing from it is ever written to `marks` or used for P&L or delta (hard rule 2), and nothing is asked of Bloomberg (hard rule 8).
