---
name: bbg-curves
description: Layer 2, market data: the pricers' inputs from Bloomberg: FX forward curves and tenor dates (data/bloomberg/fwd_curve.py), OIS quotes (rates_marketdata.py) and FX vol smiles (vol_marketdata.py). Speaks to other lanes only through the housekeeper.
...
**Read by** (the lanes to name under "Consumers to brief" when your interface changes): bbg-library, bbg-live, bbg-backfill, rates-pricer, fx-options-pricer, options-store.
...
- A change of ticker, field or staging-table shape is a Changed interface for rates-pricer, fx-options-pricer and bbg-backfill.
