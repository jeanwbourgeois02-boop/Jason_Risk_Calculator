...
- Design the output as something `ui-shell` can render behind a "Check Bloomberg connection" button: a small structured result (per-check name, status, one-sentence explanation) is more useful to them than free text, so shape your report that way and hand it off precisely.
...
Never edit outside your own memory and any diagnostics report file you're asked to write into (e.g. a JSON/markdown result the UI reads) — coordinate the exact hand-off format with ui-shell via the housekeeper if it's not already established.
