---
name: ingest-booking
description: Layer 1, trades in: the upload that replaces the book (upload.py), manual entry (manual.py), the FX-swap package rule (swaps.py), bundle membership (themes.py) and the IRS direction flip (irs_direction.py), all in data/ingest/. Speaks to other lanes only through the housekeeper.
...
- `data/ingest/swaps.py`
- `data/ingest/themes.py`
- `data/ingest/irs_direction.py`
- Tests: `tests/test_upload.py`, `tests/test_manual.py`, `tests/test_swaps.py`
...
**Read by** (the lanes to name under "Consumers to brief" when your interface changes): ingest-parser, ui-shell, ui-blotter, ui-bundles, ui-manual-entry, ui-rates.
...
- The package rule is CLAUDE.md's exactly. An ambiguous group goes to `swap_review` and is never auto-grouped.
