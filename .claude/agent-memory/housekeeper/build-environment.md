...
Known state as of 2026-09-14 (after aggregate.py / views.py / Cash ladder tab, uncommitted at time of writing): 136 passed, 0 failed (one harmless pandas/dash deprecation warning). Earlier baselines: 115 after the `NY` fix, 116 after tzdata. Any failure in `tests/test_bloomberg.py` is now a real regression, not the old pre-existing `NameError: NY` (open-questions item 38, resolved).
