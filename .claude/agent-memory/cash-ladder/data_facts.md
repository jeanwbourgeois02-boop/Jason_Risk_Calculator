...
- `engine/ladder/ladder.py::cash_ladder` (the SQL grid) reads only `trade_legs` where `settles_cash = 1`, so NDF currencies never appear in THAT function. The NDF list is `data.ingest.common.NDF_CCYS` = BRL, TWD, KRW, IDR and, since 2026-09-21, INR (`bnp.NDF_CCYS` is long gone). TRY and MXN are deliverable. The Ladder TAB's grid is a different path (`exposure_adapter.records_from_db`) and does show NDF legs: see [[ndf-fixing-dates-and-1m-price-2026-09-21]].
