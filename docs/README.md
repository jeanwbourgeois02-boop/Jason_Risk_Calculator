...
database, runs the test suite, installs the `chelsea` command into your PowerShell profile, and runs `doctor`.
...
It ends with: `Ready. Open a PowerShell terminal and type: chelsea`.
...
- On the Bloomberg PC, run `chelsea` and log in to the Terminal (in either order). The console says
...
close, in the background, without blocking the UI. Nothing runs at `chelsea` / `start` or on a timer. Progress
...
(from the project folder, or `chelsea` first and then open a new terminal there.)
...
| `'chelsea' is not recognized` | Open a **new** PowerShell window (profiles load at startup), or re-run `1_setup.cmd`. |
...
| Page shows an old `build` tag | The `GitHub:` console line says why it did not update (offline, unpushed commits). Fix that and run `chelsea` again. |
