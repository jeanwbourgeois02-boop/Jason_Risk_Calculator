...
- **Multi-curve support.** `discount_rate` and `forecast_rate` are
  independent optional inputs across `swaption.py`/`cap_floor.py`/
  `bermudan_swaption.py` (both default to `r` when omitted, so every
  single-rate call site keeps working unchanged). `discount_rate` drives
  discounting; `forecast_rate` drives only the floating-index/caplet
  forecast -- making `delta` (forecast-rate sensitivity) and `rho`
  (discount-rate sensitivity) genuinely separable for the first time (see
  `finite_difference_multi_curve_greeks`).
- **Curve-input support (no longer flat-only).** Every builder/pricer in
  `rates/` also accepts `discount_curve` / `forecast_curve` --
  `ql.YieldTermStructureHandle` overrides that, when given, are used
  DIRECTLY instead of building a `FlatForward` from `discount_rate`/
  `forecast_rate`/`r` (mutually exclusive with the matching flat-rate
  argument -- passing both raises `ValueError`). This is how a caller
  (e.g. a real bootstrapped OIS curve from elsewhere in a host project)
  feeds a genuinely SHAPED curve into these pricers -- the module itself
  still does not bootstrap anything from raw market instruments (bills,
  futures, swaps); it only accepts a curve object built elsewhere and uses
  it as-is. **Greeks under curve inputs use a different bump mechanism**:
  `finite_difference_curve_greeks` bumps a curve via a parallel zero-rate
  spread (`ql.ZeroSpreadedTermStructure` fed a `ql.SimpleQuote`), not by
  rebuilding a `FlatForward` at `rate +- h` (which only makes sense for a
  flat curve) -- verified numerically identical to the old rebuild-based
  bump in the flat case. **Still not modeled, whether flat or curve
  input:** no convexity adjustment, no CMS measure change, no
  cross-currency basis -- these are properties of the pricing model
  itself (Black-76 on a par-swap-rate proxy), not of the curve shape, so
  a shaped curve input does not add them.
- **Evaluation date is explicit and always restored.** Every
  self-contained pricing entry point accepts an optional `evaluation_date`
  (a `ql.Date`; `None` reproduces the historical "evaluate as of today")
  and restores whatever `ql.Settings.instance().evaluationDate` was set to
  before the call, on exit (`evaluation_date_scope`). Previously every
  call reset this process-wide singleton to `ql.Date.todaysDate()` and
  left it there, silently poisoning any caller-held curve/term structure
  built at a different evaluation date if a `rates/` pricer call happened
  in between -- a real bug found integrating this module into a caller
  that caches curves by `as_of` date.
...
- **Known limitations:** `hw_mean_reversion`/`hw_volatility` (Hull-White's
  ...
  basket of co-terminal European swaptions first. `tree_steps` controls
  discretization error with no auto-convergence check (default 100 is
  reasonable-but-arbitrary, not tuned per input).
- **Exercise dates: mechanically generated by default, or explicit.** The
  original behavior -- evenly spaced dates from `exercise_frequency`, not
  read off a real deal's actual coupon schedule -- is still the default.
  `price_bermudan_swaption` now also accepts an explicit `exercise_dates`
  list (`datetime.date` or `ql.Date`), bypassing the generated schedule
  entirely so a caller can match a real deal's actual exercise/coupon
  dates. Validated (`_validate_explicit_exercise_dates`): non-empty,
  strictly increasing, all strictly before the underlying swap's maturity,
  first on or after the evaluation date -- `ValueError` otherwise, never
  silently reordered or clipped. `price_european_swaption_hw` has no
  equivalent parameter (a European swaption has only one exercise date,
  so there is no schedule to override).
...
- Rates: SABR calibration to market data (parameters are supplied
  ...
  `rates/` pricers now ACCEPT a caller-supplied `discount_curve`/
  `forecast_curve` (see `_engine.py`'s CURVE-INPUT SUPPORT section) --
  what remains out of scope is this package bootstrapping one itself from
  raw market instruments (bills, futures, swaps); still no convexity
  adjustment, CMS measure change, or cross-currency basis regardless of
  whether a flat rate or a genuine curve is fed in; SABR (`sabr.py`)
  remains unshifted (no negative-rate support).
